...
public class Node extends CreaturePart {
    ...
    public Node(float sim_x, float sim_y, float width, boolean isStationary, Color renderColour) {
        ...
        this.mass_kg = (float) ((Math.PI * Math.pow(radius, 2.0)) / 5F);
    ...
    public void simStepUpdate(long stepMillis) {
        if(!isStationary) {
            this.forces = this.forces.add(new Vector(0, -9.81).times(mass_kg));
            this.velocities = this.velocities.add(this.forces.times(mass_kg * ((stepMillis*GameEngine.SIM_MULTIPLIER)/100F)));
            this.velocities = this.velocities.times(0.99);
            this.sim_pos = this.sim_pos.add(velocities.times((stepMillis*GameEngine.SIM_MULTIPLIER)/100F));
            if (isOnGround()) {
                this.sim_pos.setY(2);
                this.velocities.setX(this.velocities.getX() * 0.3);
                if (this.velocities.getY() < 0) {
                    this.velocities.setY(this.velocities.getY() * -0.75);
                }
            }
